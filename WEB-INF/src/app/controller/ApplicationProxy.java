...
public class ApplicationProxy {
	...
	public void includeAction() throws IOException {
		text("application/javascript",new TplCache(0) {
			@Override
			protected String execution() {
				try {
				    String file = null;
				    if(parseString("file",null) == null){
				       file = parseString("_ARGS").replace(".js", ".tpl.js").replaceFirst("\\?_dc=[0-9]{1,}", "");
				    }else{
				       file = parseString("file").replace(".js", ".tpl.js").replaceFirst("\\?_dc=[0-9]{1,}", "");
				    }				    
					return tpl(g.options.get("_path") + "/" + contro.getName() + "/" + file);
				} catch (IOException e) {
	...
	public void templateAction() throws IOException {
		text(new TplCache(0) {
			@Override
			protected String execution() {
				try {
				    String file = null;
				    if(parseString("file",null) == null){
				       file = parseString("_ARGS").replace(".html", ".tpl.html").replaceFirst("\\?_dc=[0-9]{1,}", "");
				    }else{
				       file = parseString("file").replace(".html", ".tpl.html").replaceFirst("\\?_dc=[0-9]{1,}", "");
				    }
					return tpl(g.options.get("_path") + "/" + contro.getName() + "/" + file);
				} catch (IOException e) {
					log.error(e.getMessage(), e);
